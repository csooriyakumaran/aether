...
static void test_alloc_basic(void)
{
    ...
    ASSERT(arena != 0);
    ASSERT(arena->pos == AETHER_ARENA_HEADER_SIZE);
...
static void test_pop_to_and_pop(void)
{
    ...
    arena_pop(arena, (u64)-1); /* absurdly large amount clamps at the header floor, doesn't underflow */
    ASSERT(arena->pos == AETHER_ARENA_HEADER_SIZE);
...
static void test_commit_chunked_flag(void)
{
    ...
    u64 pagesize    = os_mem_pagesize();
    u32 granularity = 4;
    u64 chunk       = pagesize * granularity;
    Arena* arena = arena_alloc_ex(MB(1), 0, granularity, ArenaFlags_CommitChunked);

    arena_push_array(arena, u8, 8); /* lands inside the header page committed at alloc */
    ASSERT(arena->commit_size == pagesize); /* no chunk pulled yet */

    arena_push_array(arena, u8, pagesize); /* crosses the initial commit -> chunked commit */
    ASSERT(arena->commit_size == chunk);

    arena_push_array(arena, u8, chunk - arena->pos - 1); /* one byte shy of the chunk end */
    ASSERT(arena->commit_size == chunk); /* still inside chunk 1 */

    arena_push_array(arena, u8, 2); /* tips over into chunk 2 */
    ASSERT(arena->commit_size == chunk * 2);
...
static void test_debug_fill_on_clear_flag(void)
{
    ...
    ASSERT(bytes_all_eq((u8*)arena + mark, 64, 0xDD));
...
static void test_overflow_guard(void)
{
    ...
    void* exact = arena_push(arena, arena->reserved_size - arena->pos, 1, 0);
    ASSERT(exact != 0);
...
static void test_release(void)
{
    ...
    arena_release(NULL); /* must not crash */

    Arena* arena = arena_alloc(KB(64));
    arena_push_array(arena, u8, 128);

    arena_release(arena);
    /* the header lived inside the reservation we just unmapped: the pointer
       now dangles and there is nothing observable left to assert. */
    ASSERT(1);
}
...
static void test_many_small_pushes(void)
{
    ...
    ASSERT(arena->pos == AETHER_ARENA_HEADER_SIZE + (u64)n * sizeof(u32));
    ...
    arena_clear(arena);
    ASSERT(arena->pos == AETHER_ARENA_HEADER_SIZE);
    ASSERT(arena->commit_size == pagesize); /* Decommit walks pages back down, but the header page must stay resident */
...
static void test_header_survives_clear(void)
{
    SECTION("header: clear with DebugFillOnClear poisons only above the header floor");

    u64 pagesize = os_mem_pagesize();
    Arena* arena = arena_alloc_ex(MB(1), 0, 1, ArenaFlags_DebugFillOnClear);
    u64 reserved = arena->reserved_size;

    arena_push_array(arena, u8, 256);
    arena_clear(arena);

    /* the 0xDD fill must start at the floor, not before it: bookkeeping survives */
    ASSERT(arena->pos == AETHER_ARENA_HEADER_SIZE);
    ASSERT(arena->reserved_size == reserved);
    ASSERT(arena->commit_size >= pagesize);

    u8* p = arena_push_array(arena, u8, 8); /* allocator still functions after clear */
    ASSERT(p == (u8*)arena + AETHER_ARENA_HEADER_SIZE);

    arena_release(arena);
}

static void test_first_push_at_header_floor(void)
{
    SECTION("header: first allocation lands exactly at the header floor, 128-byte aligned");

    Arena* arena = arena_alloc(KB(64));

    u8* p = arena_push_t(arena, u8);
    ASSERT((uintptr_t)p - (uintptr_t)arena == AETHER_ARENA_HEADER_SIZE);
    ASSERT(((uintptr_t)p & (AETHER_ARENA_HEADER_SIZE - 1)) == 0);
...
static TestCase g_cases[] = {
    ...
    {"first_push_at_header_floor", test_first_push_at_header_floor},
};
